...
local function adjust_levels(seg, diff)
  ...
  for _, l in ipairs(seg) do
    ...
    if stars then
      res[#res + 1] = string.rep('*', #stars + diff) .. rest
    else
      res[#res + 1] = l
    end
  end
...
local function reload_if_open(path)
  for _, b in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_get_name(b) == path and not vim.api.nvim_buf_get_option(b, 'modified') then
...
local function open_picker(params, on_done)
  ...
  if customPickers and customPickers.highlight_groups then
    customPickers.highlight_groups()
  end

  local finders, conf, actions, action_state =
    require('telescope.finders'), require('telescope.config').values, require('telescope.actions'), require('telescope.actions.state')

  local all_headlines = scanner.scan() -- { file, line, level, text, ... }

  local files_set = {}
  for _, h in ipairs(all_headlines) do
    files_set[h.file] = true
  end
  local files = {}
    ...
    return telescope_pickers.new({}, {
      ...
      attach_mappings = function(bufnr, map)
        local function toggle()
          ...
          local p = (mode == 'file') and build_picker(make_file_entries(), 'Select Target File')
