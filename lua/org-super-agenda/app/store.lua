...
function Store.set_opts(opts)
  Store.state.opts = opts or {}
end
function Store.set_cursor(cur)
  Store.state.cursor = cur
end
function Store.toggle_dupes()
  Store.state.allow_duplicates = not Store.state.allow_duplicates
end
function Store.set_view_mode(m)
  Store.state.view_mode = m or 'classic'
end
function Store.set_fullscreen(v)
  Store.state.fullscreen = v == true
end
function Store.hide(key)
  Store.state.hidden[key] = true
end
function Store.reset_hidden()
  Store.state.hidden = {}
end
...
function Store.mark_toggle(key)
  if Store.state.marked[key] then
    Store.state.marked[key] = nil
  else
    Store.state.marked[key] = true
  end
end
...
function Store.is_marked(key)
  return Store.state.marked[key] == true
end
function Store.get_marked()
  local t = {}
  for k in pairs(Store.state.marked) do
    t[#t + 1] = k
  end
  return t
end
...
function Store.get_active_view()
  return Store.state.active_view
end
function Store.get_active_view_key()
  return Store.state.active_view_key
end

function Store.push_undo(fn)
  table.insert(Store.state.undo_stack, fn)
end
...
function Store.sticky_add(key)
  Store.state.sticky_done[key] = true
end
function Store.sticky_remove(key)
  Store.state.sticky_done[key] = nil
end
function Store.sticky_has(key)
  return Store.state.sticky_done[key] == true
end
function Store.sticky_reset()
  Store.state.sticky_done = {}
end
