...
local function prio_rank(p)
  return ({ A = 3, B = 2, C = 1 })[p or ''] or 0
end
local function cmp(num, op, ref)
  if num == nil or not op or ref == nil then
    return true
  end
  if op == '<' then
    return num < ref
  ...
  elseif op == '>=' then
    return num >= ref
  elseif op == ':' then
    return tostring(num) == tostring(ref)
  else
    return num == ref
  end
end
...
function Q.parse(q)
  ...
  for tok in q:gmatch('%S+') do
    ...
    if t:match('^tag:') then
      for _, v in ipairs(splitbar(t:sub(5))) do
        (neg and P.tags_exc or P.tags_inc)[v:lower()] = true
      end
    elseif t:match('^file:') then
      P.file_inc[#P.file_inc + 1] = t:sub(6):lower()
    elseif t:match('^todo:') then
      for _, v in ipairs(splitbar(t:sub(6))) do
        P.todo[v] = true
      end
    ...
    elseif t:match('^due[<>=]') then
      local op, n = t:match('^due([<>=]=?)(%-?%d+)')
      P.due = { op = op, n = tonumber(n) }
    elseif t:match('^sched[<>=]') or t:match('^sched=') then
      local op, n = t:match('^sched([<>=]=?)(%-?%d+)')
      P.sched = { op = op, n = tonumber(n) }
  ...
  local function matches(it)
    ...
    -- include tokens
    for _, w in ipairs(P.inc) do
      if not text:find(w, 1, true) then
        return false
      end
    end
    -- exclude tokens
    for _, w in ipairs(P.exc) do
    ...
    if next(P.tags_inc) then
      ...
      for t, _ in pairs(P.tags_inc) do
        for _, it_tag in ipairs(it.tags or {}) do
          if it_tag:lower() == t then
            has = true
          end
        end
      end
    ...
    if #P.file_inc > 0 then
      ...
      for _, sub in ipairs(P.file_inc) do
        if not fn:find(sub, 1, true) then
          return false
        end
      end
    end
    -- todo
    if next(P.todo) and not P.todo[it.todo_state or ''] then
      return false
    end
    -- prio
    if P.prio.op then
      if not cmp(prio_rank(it.priority), P.prio.op, prio_rank(P.prio.val)) then
        return false
      end
    end
    -- due/sched (relative in days)
    local function days(x)
      return x and x:days_from_today() or nil
    end
    if P.due then
      local d = days(it.deadline)
      if d == nil or not cmp(d, P.due.op, P.due.n) then
        return false
      end
    end
    if P.sched then
      local d = days(it.scheduled)
      if d == nil or not cmp(d, P.sched.op, P.sched.n) then
        return false
      end
    end
    -- absolute before/after
    if P.before and not (it.deadline and it.deadline:to_time() < P.before:to_time()) then
      return false
    end
    if P.after and not (it.scheduled and it.scheduled:to_time() > P.after:to_time()) then
      return false
    end
    -- flags
    if P.is_overdue ~= nil then
      local over = (it.deadline and it.deadline:is_past()) or (it.scheduled and it.scheduled:is_past())
      if P.is_overdue and not over then
        return false
      end
      if not P.is_overdue and over then
        return false
      end
    end
    if P.is_done ~= nil then
      local done = (it.todo_state == 'DONE')
      if P.is_done and not done then
        return false
      end
      if not P.is_done and done then
        return false
      end
    end
    if P.has_todo ~= nil then
      local has = it.todo_state ~= nil and it.todo_state ~= ''
      if P.has_todo and not has then
        return false
      end
      if not P.has_todo and has then
        return false
      end
    end
    return true
