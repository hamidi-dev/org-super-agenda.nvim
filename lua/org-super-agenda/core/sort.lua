...
local key_getters = {
  ...
  deadline = function(it)
    return it.deadline and it.deadline:days_from_today() or math.huge
  end,
  scheduled = function(it)
    return it.scheduled and it.scheduled:days_from_today() or math.huge
  end,
  deadline_time = function(it)
    return it.deadline and it.deadline:to_time() or math.huge
  end,
  scheduled_time = function(it)
    return it.scheduled and it.scheduled:to_time() or math.huge
  end,
  priority = function(it)
    return prio_rank(it.priority)
  end,
  todo = function(_, ranker)
    return function(it)
      return ranker(it.todo_state)
    end
  end,
  filename = function(it)
    return (it.file or ''):match('[^/]+$') or ''
  end,
  headline = function(it)
    return (it.headline or ''):lower()
  end,
}
...
function S.sort_items(items, spec, cfg)
  ...
  local todo_rank = (by == 'todo' and spec.todo_order) and todo_rank_from_order(spec.todo_order) or todo_rank_factory(cfg)
