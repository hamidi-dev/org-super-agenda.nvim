...
function V.resolve(view_def)
  if not view_def then return nil end
  local resolved = {
...
function V.apply_filter(items, resolved_view)
  if not resolved_view or not resolved_view.filter then return items end
  local t = {}
  for _, it in ipairs(items) do
    if resolved_view.filter.matches(it) then t[#t + 1] = it end
  end
  return t
end
