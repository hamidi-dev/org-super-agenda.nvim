...
describe('group.group_items', function()
  it('groups items by matcher', function()
    ...
    local spec = {
      groups = {
        { name = 'Work', matcher = function(it) return it:has_tag('work') end },
        { name = 'Personal', matcher = function(it) return it:has_tag('personal') end },
      }
    }
  ...
  it('preserves sort field in group', function()
    ...
    local spec = {
      groups = {
        { name = 'Test', matcher = function() return true end, sort = { by = 'priority', order = 'desc' } }
      }
    }
    local groups = Group.group_items(items, spec)
  ...
  it('places item in first matching group when duplicates disabled', function()
    ...
    local spec = {
      ...
      groups = {
        { name = 'Work', matcher = function(it) return it:has_tag('work') end },
        { name = 'Urgent', matcher = function(it) return it:has_tag('urgent') end },
      }
    }
    local groups = Group.group_items(items, spec)
    assert.equals(1, #groups[1].items)
  ...
  it('excludes DONE items from Other group', function()
    ...
    local spec = {
      show_other = true,
      groups = {
        { name = 'Active', matcher = function(it) return it.todo_state == 'TODO' end },
      }
    }
    local groups = Group.group_items(items, spec)
    assert.equals(2, #groups)
